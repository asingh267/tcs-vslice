...
public class TcsAssembly extends BaseAssembly {
	...
	public TcsAssembly(Component.AssemblyInfo info, ActorRef supervisor) {
		...
		System.out.println("Inside TcsAssembly Constructor");
	...
	private AssemblyContext initialize(Component.AssemblyInfo info) {
		try {
			...
			System.out.println("Inside TcsAssembly initialize: Connections: " + info.connections());
	...
	private PartialFunction<Object, BoxedUnit> initializingReceive() {
		return locationReceive().orElse(ReceiveBuilder.matchEquals(Running, location -> {
			...
			System.out.println("Inside TcsAssembly initializingReceive");
			context().become(runningReceive());
		}).matchAny(t -> System.out.println("Unexpected message in TcsAssembly:initializingReceive: " + t)).build());
	}
	...
	private PartialFunction<Object, BoxedUnit> locationReceive() {
		return ReceiveBuilder.match(Location.class, location -> {
			if (location instanceof ResolvedAkkaLocation) {
				...
				System.out.println("Inside TcsAssembly locationReceive: actorRef: " + l.getActorRef());
				referedActor = l.getActorRef();
				supervisor.tell(Initialized, self());

			} else if (location instanceof ResolvedHttpLocation) {
				System.out.println("Inside TcsAssembly locationReceive: HTTP Service is: " + location.connection());

			} else if (location instanceof ResolvedTcpLocation) {
				ResolvedTcpLocation t = (ResolvedTcpLocation) location;
				System.out.println("Inside TcsAssembly locationReceive: TCP Location is: " + t.connection());

				if (location.connection().equals(IEventService.eventServiceConnection())) {
					System.out.println("Inside TcsAssembly locationReceive: ES connection is: " + t);
					eventService = Optional.of(IEventService.getEventService(t.host(), t.port(), context().system()));
					System.out.println("Inside TcsAssembly Event Service at: " + eventService);
				}

				if (location.connection().equals(ITelemetryService.telemetryServiceConnection())) {
					System.out.println("Inside TcsAssembly locationReceive: TS connection is: " + t);
					telemetryService = Optional
							.of(ITelemetryService.getTelemetryService(t.host(), t.port(), context().system()));
					System.out.println("Inside TcsAssembly Telemetry Service at: " + telemetryService);
				}

				if (location.connection().equals(IAlarmService.alarmServiceConnection(IAlarmService.defaultName))) {
					System.out.println("Inside TcsAssembly locationReceive: AS connection is: " + t);
				}

			} else if (location instanceof Unresolved) {
				System.out.println("Inside TcsAssembly locationReceive: Unresolved location: " + location.connection());
				...
				System.out.println("Inside TcsAssembly locationReceive: UnTracked location: " + location.connection());

			} else {
				System.out.println("Inside TcsAssembly locationReceive: Unknown connection: " + location.connection()); // XXX
			}
		}).build();
	}
