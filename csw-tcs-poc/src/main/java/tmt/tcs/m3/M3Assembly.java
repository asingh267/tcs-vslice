...
public class M3Assembly extends BaseAssembly {
	...
	public M3Assembly(Component.AssemblyInfo info, ActorRef supervisor) {
		...
		System.out.println("Inside M3Assembly Constructor");
	...
	private AssemblyContext initialize(Component.AssemblyInfo info) {
		try {
			...
			System.out.println("Inside M3Assembly initialize: Connections: " + info.connections());
			...
			diagPublsher = context()
					.actorOf(M3DiagPublisher.props(assemblyContext, m3Hcd, Optional.of(eventPublisher)));
	...
	private PartialFunction<Object, BoxedUnit> initializingReceive() {
		return locationReceive().orElse(ReceiveBuilder.matchEquals(Running, location -> {
			...
			System.out.println("Inside M3Assembly initializingReceive");
			context().become(runningReceive());
		}).matchAny(t -> System.out.println("Unexpected message in M3Assembly:initializingReceive: " + t)).build());
	}
	...
	private PartialFunction<Object, BoxedUnit> locationReceive() {
		return ReceiveBuilder.match(Location.class, location -> {
			if (location instanceof ResolvedAkkaLocation) {
				...
				System.out.println("Inside M3Assembly locationReceive: actorRef: " + l.getActorRef());
				m3Hcd = l.getActorRef();
				supervisor.tell(Initialized, self());

			} else if (location instanceof ResolvedHttpLocation) {
				System.out.println("Inside M3Assembly locationReceive: HTTP Service is: " + location.connection());

			} else if (location instanceof ResolvedTcpLocation) {
				ResolvedTcpLocation t = (ResolvedTcpLocation) location;
				System.out.println("Inside M3Assembly locationReceive: TCP Location is: " + t.connection());

				if (location.connection().equals(IEventService.eventServiceConnection())) {
					System.out.println("Inside M3Assembly locationReceive: ES connection is: " + t);
					eventService = Optional.of(IEventService.getEventService(t.host(), t.port(), context().system()));
					System.out.println("Inside M3Assembly Event Service at: " + eventService);
				}

				if (location.connection().equals(ITelemetryService.telemetryServiceConnection())) {
					System.out.println("Inside M3Assembly locationReceive: TS connection is: " + t);
					telemetryService = Optional
							.of(ITelemetryService.getTelemetryService(t.host(), t.port(), context().system()));
					System.out.println("Inside M3Assembly Telemetry Service at: " + telemetryService);
				}

				if (location.connection().equals(IAlarmService.alarmServiceConnection(IAlarmService.defaultName))) {
					System.out.println("Inside M3Assembly locationReceive: AS connection is: " + t);
				}

			} else if (location instanceof Unresolved) {
				System.out.println("Inside M3Assembly locationReceive: Unresolved location: " + location.connection());
				...
				System.out.println("Inside M3Assembly locationReceive: UnTracked location: " + location.connection());

			} else {
				System.out.println("Inside M3Assembly locationReceive: Unknown connection: " + location.connection()); // XXX
			}
		}).build();
	}
