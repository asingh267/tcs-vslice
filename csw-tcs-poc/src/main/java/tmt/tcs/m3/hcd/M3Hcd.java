...
public class M3Hcd extends BaseHcd {
	...
	private M3Hcd(final Component.HcdInfo info, ActorRef supervisor) throws Exception {
		System.out.println("Inside M3Hcd");
	...
	public void process(SetupConfig sc) {
		System.out.println("Inside M3Hcd process received sc: " + sc);

		CurrentState m3State;
		ConfigKey configKey = sc.configKey();

		if (configKey.equals(M3Config.moveCK)) {
			System.out.println("Inside M3Hcd process received move command");
			m3State = jadd(M3Config.defaultM3StatsState, jset(M3Config.az, 1.0));
		} else {
			System.out.println("Inside M3Hcd process received offset command");
			m3State = jadd(M3Config.defaultM3StatsState, jset(M3Config.az, 2.0));
		}
