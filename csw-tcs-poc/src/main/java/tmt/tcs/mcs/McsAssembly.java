...
public class McsAssembly extends BaseAssembly {
	...
	public McsAssembly(Component.AssemblyInfo info, ActorRef supervisor) {
		...
		System.out.println("Inside McsAssembly Constructor");
	...
	private AssemblyContext initialize(Component.AssemblyInfo info) {
		try {
			...
			System.out.println("Inside McsAssembly initialize: Connections: " + info.connections());
			...
			diagPublsher = context()
					.actorOf(McsDiagPublisher.props(assemblyContext, mcsHcd, Optional.of(eventPublisher)));
	...
	private PartialFunction<Object, BoxedUnit> initializingReceive() {
		return locationReceive().orElse(ReceiveBuilder.matchEquals(Running, location -> {
			...
			System.out.println("Inside McsAssembly initializingReceive");
			context().become(runningReceive());
		}).matchAny(t -> System.out.println("Unexpected message in McsAssembly:initializingReceive: " + t)).build());
	}
	...
	private PartialFunction<Object, BoxedUnit> locationReceive() {
		return ReceiveBuilder.match(Location.class, location -> {
			if (location instanceof ResolvedAkkaLocation) {
				...
				System.out.println("Inside McsAssembly locationReceive: actorRef: " + l.getActorRef());
				mcsHcd = l.getActorRef();
				supervisor.tell(Initialized, self());

			} else if (location instanceof ResolvedHttpLocation) {
				System.out.println("Inside McsAssembly locationReceive: HTTP Service is: " + location.connection());

			} else if (location instanceof ResolvedTcpLocation) {
				ResolvedTcpLocation t = (ResolvedTcpLocation) location;
				System.out.println("Inside McsAssembly locationReceive: TCP Location is: " + t.connection());

				if (location.connection().equals(IEventService.eventServiceConnection())) {
					System.out.println("Inside McsAssembly locationReceive: ES connection is: " + t);
					eventService = Optional.of(IEventService.getEventService(t.host(), t.port(), context().system()));
					System.out.println("Inside McsAssembly Event Service at: " + eventService);
				}

				if (location.connection().equals(ITelemetryService.telemetryServiceConnection())) {
					System.out.println("Inside McsAssembly locationReceive: TS connection is: " + t);
					telemetryService = Optional
							.of(ITelemetryService.getTelemetryService(t.host(), t.port(), context().system()));
					System.out.println("Inside McsAssembly Telemetry Service at: " + telemetryService);
				}

				if (location.connection().equals(IAlarmService.alarmServiceConnection(IAlarmService.defaultName))) {
					System.out.println("Inside McsAssembly locationReceive: AS connection is: " + t);
				}

			} else if (location instanceof Unresolved) {
				System.out.println("Inside McsAssembly locationReceive: Unresolved location: " + location.connection());
				...
				System.out.println("Inside McsAssembly locationReceive: UnTracked location: " + location.connection());

			} else {
				System.out.println("Inside McsAssembly locationReceive: Unknown connection: " + location.connection()); // XXX
			}
		}).build();
	}
	...
	public List<Validation.Validation> setup(SetupConfigArg sca, Optional<ActorRef> commandOriginator) {
		System.out.println("Inside McsAssembly: setup SetupConfigArg is : " + sca);
